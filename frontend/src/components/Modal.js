...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  // 2. Añadir el useEffect para manejar la tecla "Escape"
  useEffect(() => {
    // Función que se ejecuta cuando se presiona una tecla
    const handleEscape = (event) => {
      if (event.key === 'Escape') {
        onClose(); // Llama a la misma función que el botón de cerrar
      }
    };

    // Si el modal está abierto, empezamos a escuchar las teclas
    if (isOpen) {
      window.addEventListener('keydown', handleEscape);
    }

    // Función de limpieza: cuando el modal se cierra o el componente se desmonta,
    // dejamos de escuchar para no consumir recursos innecesarios.
    return () => {
      window.removeEventListener('keydown', handleEscape);
    };
  }, [isOpen, onClose]); // Este efecto se volverá a ejecutar si isOpen o onClose cambian
